...
# 7 - Defining a custom heatmap function for species intercorrelations
#
################################################################################

# This code based on the function "heatmap.2" from the package "gplots"
# Call the original function: library(gplots), heatmap.2
# Search for "change"
custom_heatmap_function <- function(x, #Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
                                    Rowv = FALSE, Colv = FALSE, # change
                                    distfun = dist, hclustfun = hclust, 
                                    #dendrogram = c("both", "row", "column", "none"), 
                                    dendrogram = "none", # change
                                    reorderfun = function(d, w) reorder(d, w), 
                                    symm = FALSE, scale = c("none", "row", "column"), 
                                    na.rm = TRUE, revC = identical(Colv, "Rowv"), 
                                    add.expr, breaks, symbreaks = 
                                      any(x < 0, na.rm = TRUE) || scale != "none", 
                                    col = "heat.colors", colsep, rowsep, sepcolor = "white", 
                                    sepwidth = c(0.05, 0.05), cellnote, na.color = par("bg"), 
                                    #notecex = 1, notecol = "cyan", 
                                    notecex = 1.5, notecol = "black", # change
                                    ...
                                    cexRow = 1.3, cexCol = 1.3, # change
                                    labRow = NULL, labCol = NULL, srtRow = NULL, 
                                    srtCol = NULL, adjRow = c(0, NA), adjCol = c(NA, 0), 
                                    offsetRow = 0.5, offsetCol = 0.5, colRow = NULL, 
                                    colCol = NULL, key = TRUE, keysize = 1.5, lmat = NULL,
                                    #density.info = c("histogram", "density", "none"), 
                                    density.info = "none", # change
                                    denscol = tracecol, symkey = 
                                      any(x < 0, na.rm = TRUE) || symbreaks, densadj = 0.25,
                                    #key.par = list(), key.title = NULL, key.xlab = NULL,
                                    key.par = list(mar = c(7, 1, 2.5, 1.5), cex = 0.9), # change (b, l, t, r)
                                    key.title = NA, key.xlab = "Spearman's rho", # change
                                    key.ylab = NULL, key.xtickfun = NULL, key.ytickfun = NULL, 
                                    main = NULL, xlab = NULL, ylab = NULL, extrafun = NULL, ...) 
{
  ...
  image(1:nc, 1:nr, x, xlim = 0.5 + c(0, nc), ylim = 0.5 + 
        ...
        breaks = breaks, ...)
  # change color of box around heatmap here
  box(lty = "solid", col = 'grey85')
  retval$carpet <- x
  if (exists("ddr")) 
  ...
  retval$col <- col
  # change the following if statement by removing "!invalid(na.color) & "
  if (any(is.na(x))) {
  ...
  if (is.null(srtCol) && is.null(colCol)) 
    # change font (bold/italic) of column labels here
    axis(1, 1:nc, labels = labCol, las = 2, line = -0.5 + 
           offsetCol, tick = 0, cex.axis = cexCol, hadj = adjCol[1], font=3,
         padj = adjCol[2])
  else {
  ...
  if (is.null(srtRow) && is.null(colRow)) {
    # change font (bold/italic) of row labels here
    axis(4, iy, labels = labRow, las = 2, line = -0.5 + offsetRow, font=3,
  ...
  if (dendrogram %in% c("both", "row")) {
    ...
    if ("try-error" %in% class(flag)) {
      ...
      if (!is.null(cond) && conditionMessage(cond) == 
        ...
        stop("Row dendrogram too deeply nested, recursion limit exceeded.  
             Try increasing option(\"expressions\"=...).")
    }
  }
  else plot.new()
  ...
  if (dendrogram %in% c("both", "column")) {
    ...
    if ("try-error" %in% class(flag)) {
      ...
      if (!is.null(cond) && conditionMessage(cond) == 
          "evaluation nested too deeply: infinite recursion / options(expressions=)?") 
        stop("Column dendrogram too deeply nested, recursion limit exceeded.  
             Try increasing option(\"expressions\"=...).")
    }
  }
  else plot.new()
  ...
  if (key) {
    ...
    # change color of box around color key here
    image(z = matrix(z, ncol = 1), col = col, breaks = tmpbreaks, 
          xaxt = "n", yaxt = "n", frame.plot=FALSE)
    box(lty = "solid", col = 'grey70')
    par(usr = c(0, 1, 0, 1))
    if (is.null(key.xtickfun)) {
      lv <- pretty(breaks)
      xv <- scale01(as.numeric(lv), min.raw, max.raw)
      # change color of x axis in color key here
      xargs <- list(at = xv, labels = lv, col="grey50")
    }
    else {
      xargs <- key.xtickfun()
    }
